...
import { completePasskeyEnrollment } from "@/lib/server/passkey-service";
...
export async function POST(request: Request) {
  try {
    ...
    if (!ceremony || ceremony.mode !== "enroll") {
      throw new Error("PASSKEY_CEREMONY_MISSING");
    }

    const payload = passkeyResponseSchema.parse(await readJson(request));
    const passkey = await completePasskeyEnrollment(auth, ceremony, payload.response as RegistrationResponseJSON);
    const response = NextResponse.json({ passkey: serializePasskeyCredential(passkey) });
